...
$mailTo = ["raspil-ds@yandex.ru", "vasilyev-r@yandex.ru", "vasilyev-r@mail.ru"];
	...
	if ($captcha->success == true && $captcha->score > 0.125) {
		...
		if (!empty($file_tmp)) {
			if (in_array($file_type, $allowed_types) && $file_size < 5120000) {
				...
				$_SESSION["win"] = 1;
				$_SESSION["recaptcha"] =
					'<p class="text-light">Вы пытаетесь загрузить неподходящий формат или размер файла. Файл должен быть в формате .jpg, .jpeg, .png, .pdf или .heic и размером не более 5 МБ. Пожалуйста повторите попытку.</p>';
		...
		$body_with_attach = $picture ? "<p><strong>В прикрепленном файле находится изображение изделия или схематично нарисованный рисунок!</strong></p>" : "";
		...
		$attachments = $picture ? [["path" => $picture, "name" => basename($picture)]] : [];
