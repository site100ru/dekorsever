...
$mailTo = ["info@dekor-sever.ru", "vasilyev-r@yandex.ru", "vasilyev-r@mail.ru"];
...
if (!empty($_POST)) {
	...
	if ($captcha->success == true && $captcha->score > 0.125) {
		...
		if (!empty($_FILES["file"]["name"][0])) {
			foreach ($_FILES["file"]["name"] as $key => $filename) {
				...
				if (in_array($file_type, $allowed_types) && $file_size < 5120000) {
					...
					$_SESSION["win"] = 1;
					$_SESSION["recaptcha"] =
						'<p class="text-light">Вы пытаетесь загрузить неподходящий формат или размер одного или нескольких файлов. Файлы должны быть в формате .jpg, .jpeg, .png, .pdf или .heic и размером не более 5 МБ каждый. Пожалуйста повторите попытку.</p>';
		...
		$attach_note = !empty($attachments) ? "<p><strong>В прикрепленных файлах находятся изображения изделия или схематично нарисованный рисунок!</strong></p>" : "";
